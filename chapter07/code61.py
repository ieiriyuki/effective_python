import asyncio, random, contextlib, math
...
async def run_async_client(address):
    ...
    async with client.session(1, 3, 5):
        results = [(x, await client.report_outcome(x))
                   async for x in client.request_numbers(5)]
    async with client.session(10, 15, 12):
        async for num in client.request_numbers(5):
            outcome = await client.report_outcome(num)
            results.append((num, outcome))
...
async def main():
    ...
    for num, outcome in results:
        print(f"Client: {num} is {outcome}")
